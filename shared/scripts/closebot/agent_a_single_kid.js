...
const logFile = path.join(logDir, 'agent_a_single_kid.log');
...
async function main() {
  log(`=== SINGLE KID baseline booking test (Kids 7-13) ===`);
  ...
  const PERSONA = "Anna Williams, 35 y/o mom. DOB: March 10, 1989. Email: anna.williams.test@example.invalid. Phone: 415-555-0111 (give as full US number with area code). You want to sign up just ONE daughter, Emma Williams, age 10, DOB December 4, 2015 — only one kid, no siblings. Emma has never trained martial arts before. Be flexible with timing — take whatever slot the bot offers. If the bot asks for a preferred day/time, say something open like 'whatever works, weekday afternoon is fine' or just confirm the first slot the bot mentions. Casual friendly mom tone.";
  const convo = [];
  const OPENING = "Hi! I'd love to sign my daughter up for a free trial class.";
  ...
  const bookingLogs = reader.events.flatMap(e => (e.type === 'logs' && Array.isArray(e.logs)) ? e.logs.filter(l => /Booking|Available|calendar|No calendar|book_appointment|SUCCESS|ERROR|Booked/i.test(l.message || '')).map(l => l.message) : []);
