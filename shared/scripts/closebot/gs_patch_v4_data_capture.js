/**
 * v4 patch — Data Capture + conversationReason + affordability pivot + drive time block + remove ns05
 *
 * Fixes:
 *  1. n20_details: disable CheckAvailability, explicit "do NOT confirm booking" instruction
 *  2. conversationReason: expand identity guard (AI/bot/automated, not just "virtual assistant")
 *  3. n10_intro Push Toward Booking: affordability pivot to free first class
 *  4. n10_intro Answer Questions: block drive time estimation
 *  5. Remove ns05_info_pressure_handoff (empty description, fires randomly, confirmed decommission)
 *
 * Usage:
 *   node --env-file=.env --env-file=clients/ground-standard/.env \
 *     shared/scripts/closebot/gs_patch_v4_data_capture.js
 */
import fs from 'fs';
...
const LOG = path.join(logDir, 'gs_patch_v4_data_capture.log');
...
async function main() {
  log('=== v4 patch: Data Capture + identity guard + affordability pivot + remove ns05 ===');

  // 1. PUT updated KDL
  log('--- 1. PUT updated KDL ---');
  const put = await api('PUT', `/bot/${BOT_ID}`, { importKdl: KDL });
  log(`  PUT → ${put.status}`);
  if (!put.ok) { log('FAIL: ' + JSON.stringify(put.json)); process.exit(1); }
  ...
  if (exp.ok && exp.json?.kdl) {
    ...
    // Check n20_details has CheckAvailability disabled
    const n20chunk = kdl.slice(kdl.indexOf('"n20_details"'), kdl.indexOf('"n30_book"'));
    ...
    // Check identity guard
    const hasAiGuard = kdl.includes('automated system');
    log(`  AI identity guard in conversationReason: ${hasAiGuard} (expected true)`);

    // Check affordability pivot
    const hasAffordability = kdl.includes('first class is on us');
    log(`  Affordability pivot present: ${hasAffordability} (expected true)`);

    // Check drive time block
    const hasDriveTime = kdl.includes('Google Maps');
    log(`  Drive time block present: ${hasDriveTime} (expected true)`);

    // Check ToolOrder removed from n20
    const n20toolOrder = n20chunk.includes('ToolOrder {');
    log(`  n20_details ToolOrder block present: ${n20toolOrder} (expected false)`);
  } else {
    log(`  WARN: export failed — ${exp.status}`);
  }

  log('\n=== Patch complete ===');
  log('Run t09_confused_parent to verify kids booking now exits to n30_book properly.');
}
