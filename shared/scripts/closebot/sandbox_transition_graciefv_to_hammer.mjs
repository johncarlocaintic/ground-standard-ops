// Sandbox transition: Gracie Farmington Valley → Hammer Sports & Performance
...
const PREV_BOT = 'bot_J7WW9BOARJK0NI9F';
const PREV_KB_ID = 'file_Y12JSQXU63UJ9Z0N';

const NEW_BOT = 'bot_AFKR1QYFJ3VKYF3W';
const NEW_KB_ID = 'file_2WNTLIZS3TRK33BF';
const NEW_CALS = [
  { name: 'Adult Brazilian Jiu-Jitsu', slotMin: 60 },
...
log('=== TRANSITION: Gracie FV → Hammer Sports ===');

log('Step 1: Detach previous bot (Gracie FV) from sandbox');
...
log('Step 2: Detach previous KB (Gracie FV) from sandbox');
...
log('Step 4: Create Hammer Sports calendars (fresh)');
const newCalIds = {};
...
log('Step 5: Attach Hammer Sports KB to sandbox');
...
log('Step 6: Attach new bot (Hammer Sports) to sandbox');
...
log('TRANSITION COMPLETE — sandbox ready for Hammer Sports sweep');
log(`HAMMER_KB_ID=${NEW_KB_ID}`);
log(`HAMMER_CAL_IDS=${JSON.stringify(newCalIds)}`);
