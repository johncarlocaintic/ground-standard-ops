/**
 * Deploy Vacaville v3.18:
 *   1) Create new bot from updated KDL (conversationReason: free-trial → first class)
 *   2) Publish
...
const LOG = path.join(logDir, 'gs_deploy_v3_18.log');
...
async function main() {
  ...
  fs.writeFileSync(stateFile, JSON.stringify({
    ...
    notes: 'v3.18: conversationReason — call it first class, never free trial.',
  }, null, 2));
  log(`  state → ${stateFile}`);
