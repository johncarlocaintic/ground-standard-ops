#!/usr/bin/env bash
BOT="bot_QDSOGLYJA9B4HIO0"
RUBRIC="shared/scripts/closebot/rubrics/signature.json"
PDIR="shared/scripts/closebot/personas/signature"
SANDBOX="src_4R4DUIQTMMX2NFPU"
SUMMARY="shared/logs/eval/_signature_sweep_summary.txt"
: > "$SUMMARY"
run () {
  ...
  rd=$(ls -t shared/logs/eval/ | grep "^signature_signature_$1_" | head -1)
  v=$(grep -m1 'Overall:' "shared/logs/eval/$rd/report.md" 2>/dev/null || echo "NO REPORT")
  echo "  -> $1 :: $rd :: $v" | tee -a "$SUMMARY"
}
for p in adult_only kid_5_7 kid_11_14 adult_and_kid teen_15_17_nocal under5_redirect minor_self_booking pricing_deflect nonbookable_private hostile_aggression; do run "$p"; done
