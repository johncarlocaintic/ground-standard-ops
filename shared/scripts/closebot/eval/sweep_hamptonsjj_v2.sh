#!/usr/bin/env bash
# Hamptons Jiu-Jitsu South v2.0 Agent Node sweep on bot_WWB97FEM611TC5SY
BOT="bot_WWB97FEM611TC5SY"
RUBRIC="shared/scripts/closebot/rubrics/hamptonsjj.json"
PDIR="shared/scripts/closebot/personas/hamptonsjj"
SANDBOX="src_4R4DUIQTMMX2NFPU"
SUMMARY="shared/logs/eval/_hamptonsjj_v2_sweep_summary.txt"
: > "$SUMMARY"
...
run () {
  ...
  rd=$(ls -t shared/logs/eval/ | grep "^hamptonsjj_hamptonsjj_$1_" | head -1)
  v=$(grep -m1 'Overall:' "shared/logs/eval/$rd/report.md" 2>/dev/null || echo "NO REPORT")
  echo "  -> $1 :: $rd :: $v" | tee -a "$SUMMARY"
}

for p in adult_only adult_muay_thai kid_only_young kid_older adult_and_kid minor_self_booking nonbookable_program pricing_deflect; do
  run "$p"
done
