#!/usr/bin/env bash
# Resume after shutdown — runs ONLY the remaining personas, APPENDS to summary
# (does not truncate the 6 valid completed runs).
BOT="bot_CXGYLTWOIJIP4ZCO"
...
SUMMARY="shared/logs/eval/_logica_sweep_summary.txt"
echo "=== RESUME AFTER SHUTDOWN $(date -u +%H:%M:%S) ===" >> "$SUMMARY"
run () {
...
for p in minor_self_booking pricing_deflect nonbookable_program hostile_aggression; do run "$p"; done
echo "=== NON-DETERMINISM: adult_only x3 ===" | tee -a "$SUMMARY"
for i in 1 2 3; do run "adult_only"; done
echo "=== RESUME SWEEP COMPLETE $(date -u +%H:%M:%S) ===" | tee -a "$SUMMARY"
