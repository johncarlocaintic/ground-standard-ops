#!/usr/bin/env bash
BOT="bot_FIWVSZBWNX546KKA"
RUBRIC="shared/scripts/closebot/rubrics/invertedgear.json"
PDIR="shared/scripts/closebot/personas/invertedgear"
SANDBOX="src_4R4DUIQTMMX2NFPU"
SUMMARY="shared/logs/eval/_invertedgear_sweep_summary.txt"
: > "$SUMMARY"
run () {
  ...
  rd=$(ls -t shared/logs/eval/ | grep "^invertedgear_invertedgear_$1_" | head -1)
  v=$(grep -m1 'Overall:' "shared/logs/eval/$rd/report.md" 2>/dev/null || echo "NO REPORT")
  echo "  -> $1 :: $rd :: $v" | tee -a "$SUMMARY"
}
for p in adult_only kid_cubs kid_juniors adult_and_kid teen_13_17_nocal \
         minor_self_booking pricing_deflect nonbookable_private; do run "$p"; done
