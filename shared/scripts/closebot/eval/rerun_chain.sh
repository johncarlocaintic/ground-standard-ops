#!/usr/bin/env bash
# Rerun chain: targeted bug-fix retests + NO_REPORT reruns across 5 gyms.
# Run after: chain sweep complete + JJ cap + handoff applied + bug fixes applied.
...
run_persona() {
  local SLUG=$1   # gym slug (used in run-id prefix)
  local BOT=$2
  local RUBRIC=$3
  local PDIR=$4
  local PERSONA=$5
...
# ---------------- Gracie FV (bug-fix verify + NO_REPORT rerun) ----------------
attach graciefv
run_persona graciefarmingtonvalley bot_J7WW9BOARJK0NI9F shared/scripts/closebot/rubrics/graciefarmingtonvalley.json shared/scripts/closebot/personas/graciefarmingtonvalley teen_14_17_nocal
run_persona graciefarmingtonvalley bot_J7WW9BOARJK0NI9F shared/scripts/closebot/rubrics/graciefarmingtonvalley.json shared/scripts/closebot/personas/graciefarmingtonvalley adult_kickboxing

# ---------------- Hammer Sports (3 non-det adult_bjj_default) ----------------
attach hammer
for i in 1 2 3; do
