#!/usr/bin/env bash
BOT="bot_J7WW9BOARJK0NI9F"
RUBRIC="shared/scripts/closebot/rubrics/graciefarmingtonvalley.json"
PDIR="shared/scripts/closebot/personas/graciefarmingtonvalley"
SANDBOX="src_4R4DUIQTMMX2NFPU"
SUMMARY="shared/logs/eval/_graciefarmingtonvalley_sweep_summary.txt"
: > "$SUMMARY"
run () {
  ...
  rd=$(ls -t shared/logs/eval/ | grep "^graciefarmingtonvalley_graciefarmingtonvalley_$1_" | head -1)
  v=$(grep -m1 'Overall:' "shared/logs/eval/$rd/report.md" 2>/dev/null || echo "NO REPORT")
  echo "  -> $1 :: $rd :: $v" | tee -a "$SUMMARY"
}
for p in adult_only adult_kickboxing kid_youngest kid_middle kid_older \
         adult_and_kid teen_14_17_nocal minor_self_booking \
         pricing_deflect hostile_aggression; do run "$p"; done
