// Sandbox transition: Hamptons Jiu-Jitsu South → Mason Dixon Jiu-Jitsu
...
const PREV_BOT = 'bot_WWB97FEM611TC5SY';
const PREV_KB_ID = 'file_R2H1G3T1XFE4NRMR';

const NEW_BOT = 'bot_0HQBLZA2NO9T1ZFM';
const NEW_KB_ID = 'file_K8E6900W9STUOHBZ';
const NEW_CALS = [
  { name: 'Adult Fundamentals BJJ', slotMin: 60 },
  { name: 'Adult Striking', slotMin: 60 },
  { name: 'Kids 4-7 Martial Arts', slotMin: 45 },
  { name: 'Kids 8-13 Martial Arts', slotMin: 45 },
];
...
log('=== TRANSITION: Hamptons JJS → Mason Dixon ===');

log('Step 1: Detach previous bot (Hamptons JJS) from sandbox');
...
log('Step 2: Detach previous KB (Hamptons JJS) from sandbox');
...
log('Step 4: Create Mason Dixon calendars (fresh)');
const newCalIds = {};
...
log('Step 5: Attach Mason Dixon KB to sandbox');
...
log('Step 6: Attach new bot (Mason Dixon) to sandbox');
...
log('TRANSITION COMPLETE — sandbox ready for Mason Dixon sweep');
log(`MASONDIXON_KB_ID=${NEW_KB_ID}`);
log(`MASONDIXON_CAL_IDS=${JSON.stringify(newCalIds)}`);
