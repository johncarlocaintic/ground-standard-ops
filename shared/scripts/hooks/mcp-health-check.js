...
// [TRIAL LOGGER — review 2026-05-04, remove block below after trial ends]
function __trialLog(event, details) {
  try {
    const line = `- ${new Date().toISOString()} — ${event}${details ? ': ' + details : ''}\n`;
    const trialFile = path.join(__dirname, '..', '..', '..', 'tasks', 'trials', 'mcp-health-check-hook.md');
    fs.appendFileSync(trialFile, line);
  } catch {}
}
// [/TRIAL LOGGER]
...
function markHealthy(state, serverName, now, details = {}) {
  const wasUnhealthy = state.servers[serverName] && state.servers[serverName].status === 'unhealthy';
  ...
  if (wasUnhealthy) __trialLog('recovered', serverName);
}
...
function markUnhealthy(state, serverName, now, failureCode, errorMessage) {
  ...
  __trialLog('unhealthy', `${serverName} (code=${failureCode}, fails=${failureCount})`);
}
...
function attemptReconnect(serverName) {
  ...
  __trialLog('reconnect attempt', serverName);
