...
function __trialLog(event, details) {
  try {
    ...
    const trialFile = path.join(__dirname, '..', '..', '..', 'tasks', 'trials', 'session-end-hook.md');
...
async function main() {
  ...
  if (fs.existsSync(sessionFile)) {
    ...
    if (updatedContent) {
      ...
      __trialLog('updated', `${path.basename(sessionFile)} (summary=${summary ? 'yes' : 'no'}, msgs=${summary ? summary.totalMessages : 0})`);
    }
...
`;
    ...
    __trialLog('created', path.basename(sessionFile));
