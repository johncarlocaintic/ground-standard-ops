...
function __trialLog(event, details) {
  try {
    ...
    const trialFile = path.join(__dirname, '..', '..', '..', 'tasks', 'trials', 'doc-file-warning-hook.md');
...
function run(inputOrRaw, _options = {}) {
  ...
  if (filePath && isSuspiciousDocPath(filePath)) {
    __trialLog('warned', filePath);
    return {
      exitCode: 0,
      stderr:
        '[TRIAL Hook] WARNING: Ad-hoc documentation filename detected\n' +
        `[TRIAL Hook] File: ${filePath}\n` +
        '[TRIAL Hook] Consider using a structured path (e.g. docs/, .claude/, skills/, .github/, benchmarks/, templates/, clients/*/docs/)',
    };
  }
